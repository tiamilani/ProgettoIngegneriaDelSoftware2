...
function placesNearby (resp, map, city, range, type, nome) {
	console.log("Devo inviare i risultati!");
	map.placesNearby(
		{
			location: city,
			radius: range,
...
exports.luoghiUtili = function(req, resp) {
	...
	//Imposto l'head http
	/*
	//Consentire una richiesta da un'altro dominio
	resp.header('Access-Control-Allow-Origin','*');
	resp.header('Access-Control-Allow-Headers', 'Origin, X-Requested-...');
	resp.header('Content-Typ','application/json');
	if(resp.method == 'OPTIONS'){
		resp.header('Access-Control-Allow-Methods', 'PUT,POST,DELETE');
		resp.status(200).json({});
	}
	*/
	resp.writeHead(200, {"Content-Type": "application/json"});

	//Ottengo le variabili che mi vengono passate tramite GET
	/*
		Valori possibili di name:
		- biblioteca
		- copisteria
		- mensa
		- facolta
		Valori possibili di place:
		- trento
		- mesiano
		- povo
		- oltrecastello
		- luogo specifico dettato dalla chiamata
	*/
